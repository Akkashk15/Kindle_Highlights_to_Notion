...
var makeRequest = require('@dnpr/make-request');
 ...
 */
const log = new logger.Logger("notionapi-agent");
...
${agentOptions.port} ${agentOptions.path}`);
            ...
            try {
                if (myURL.protocol === "http:") {
                    response = await makeRequest.makeHTTPRequest(agentOptions, payload);
                }
                else {
                    response = await makeRequest.makeHTTPSRequest(agentOptions, payload);
                }
            }
            catch (error) {
                throw error;
